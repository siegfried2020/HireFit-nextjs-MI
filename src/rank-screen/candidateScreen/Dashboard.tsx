    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between sm:gap-6">
        <div className="min-w-0">
          ...
          <h1 className="text-[20px] font-bold tracking-tight text-ink sm:text-[22px]">
            You&rsquo;re building toward QA Automation Engineer
          </h1>
        </div>
        <Button onClick={() => go("readiness")} className="w-full shrink-0 sm:w-auto max-md:hidden">
      ...
      <Card className="overflow-hidden">
        <div className="grid grid-cols-1 divide-y divide-line lg:grid-cols-[1.4fr_1fr_1fr] lg:divide-x lg:divide-y-0">
      ...
      {/* Next best action */}
      <Card className="flex flex-col gap-3.5 border-brand-200 bg-brand-50 p-4 min-[480px]:flex-row min-[480px]:items-center">
        <div className="flex min-w-0 flex-1 gap-3.5">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-0.5 text-[12.5px] text-brand-700/80">
              Estimated +6% readiness · creates verified evidence
            </div>
          </div>
        </div>
        <Button className="w-full shrink-0 min-[480px]:w-auto" onClick={() => go("readiness")}>
          Start assessment
        </Button>
      </Card>
        ...
        <Card className="min-w-0 p-4">
          ...
          <div className="space-y-2">
            {[
              ...
              <div
                key={g.s}
                className="flex items-center justify-between gap-3 rounded-[9px] border border-line-soft bg-raised px-3 py-2"
              >
                <div className="min-w-0 flex-1">
                  <div className="text-[13.5px] font-semibold text-ink">{g.s}</div>
                  <div className="text-[12px] text-muted">→ {g.to}</div>
                </div>
                <span className="shrink-0 rounded-full bg-[#e7f2ec] px-2 py-0.5 text-[12px] font-bold text-[#276c4f]">
                  {g.impact}
                </span>
              </div>
            ))}
          ...
          <div className="divide-y divide-line-soft">
            {[
              ...
              <button
                key={j.role}
                onClick={() => go("jobs")}
                className="flex w-full items-start gap-3 py-2.5 text-left transition-colors first:pt-0 last:pb-0 hover:bg-line-soft/60 min-[400px]:items-center"
              >
                <div className="grid size-8 shrink-0 place-items-center rounded-[8px] bg-brand-100 text-[11px] font-bold text-brand-700">
                  {j.co.slice(0, 2)}
                </div>
                <div className="min-w-0 flex-1">
                  <div className="text-[13px] font-semibold leading-snug text-ink">{j.role}</div>
                  <div className="text-[12px] text-muted">{j.co}</div>
                </div>
                <span className="shrink-0 pt-0.5 text-[14px] font-bold tabular-nums text-brand-600 min-[400px]:pt-0">
                  {j.match}%
                </span>
              </button>
            ))}
          ...
          <div className="space-y-3">
            {[
              ...
              <div key={u.t} className="flex gap-2.5">
                ...
                <div className="min-w-0 flex-1">
                  <div className="text-[13px] font-semibold leading-snug text-ink">{u.t}</div>
            ...
            <button className="flex w-full items-center gap-2.5 rounded-[9px] border border-dashed border-line px-3 py-2 text-left text-[12.5px] font-medium text-muted transition-colors hover:border-brand-200 hover:text-brand-600">
              <Mic className="size-4 shrink-0" /> Start Voice Career Discovery
            </button>
          </div>
        </Card>
      </div>
